...
using ChamoAligned = Container<Type, Eigen::aligned_allocator<Type>>;
...
struct InvertedFile {
  ...
  ChamoAligned<std::vector, Descriptor> descriptors_;
...
void AddDescriptor(
    ...
    ChamoAligned<std::vector, InvertedFile<DescType, DescDim> >* inverted_files) {
  CHECK_NOTNULL(word_index_map);
  CHECK_NOTNULL(inverted_files);

  ChamoAligned<std::vector, InvertedFile<DescType, DescDim> >& inverted_files_ref =
      *inverted_files;
